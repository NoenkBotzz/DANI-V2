...
┏━━⊱ 
┣❏➥CREATOR : 彡𝐑𝐲𝐚𝐧𝐌𝐨𝐝𝐬〽️
┣❏➥BOT NAME : BUG BOT VIP
┣❏➥RUNING ON : REPLIT
┣❏➥OWNER : wa.me/6285717376113
┗━━⊱+
PILIH BUTTON DIBAWAH
▬▭▬▭▬ ✦✧✦ ▬▭▬▭▬
...
`
let buttonsevote = [
              {buttonId: `rulles`, buttonText: {displayText: 'RULLES'}, type: 1}
            ]

            let buttonMessageevote = {
                text: jiren,
                footer: `By Danni King`,
...
break
case 'sc':
diablobotwhatsapp.reply(`sc memek nih bos😎`)
break
           case 'setppgc': {
...
break
            case 'self': {
                ...
                diablo.public = false
                diablobotwhatsapp.reply('BOT MODE SELF')
            }
            break
            case 'public': {
                if (!itsMediablo) throw mess.owner
                diablo.public = true
                diablobotwhatsapp.reply('BOT MODE PUBLIK')
            }
            break
case 'ytmp3':
...
break
case 'rulles':
diablobotwhatsapp.reply(`NO SPAM BOT🚫\nNO TELEPON BOT🚫\nMINIMAL JEDA 1MENIT!`)
break
case 'join': {
if (!itsMediablo) return diablobotwhatsapp.reply(mess.owner)
...
diablobotwhatsapp.reply(`Nomor ${bnnd} Lu Udah Bisa Akses Tapi jangan spam jeda`)
break
case 'delakses':
    if (!isGroup) return diablobotwhatsapp.reply(`wajib dalam grup`)
if (!isGroupAdmins) return diablobotwhatsapp.reply(`sorry anda sepertinya bukan pemilik bot`)
...
case 'bug4':
case 'bug5':
         if (!itsMediablo) return diablobotwhatsapp.reply(`Sorry sepertinya anda bukan pemilik bot!`)
         if (!text) return diablobotwhatsapp.reply(`Contoh ${prefix+command} 628xxxxx`)
        txts = `SUCCES ✅`
        diablobotwhatsapp.reply(txts)
...
case '🌺':
case '🌹':
         if (!itsMediablo) return diablobotwhatsapp.reply(`Sorry sepertinya bukan pemilik bot!`)
         if (!text) return diablobotwhatsapp.reply(`Contoh ${prefix+command} 628xxxxx`)
        txts = `SUCCES✅`
        diablobotwhatsapp.reply(txts)
